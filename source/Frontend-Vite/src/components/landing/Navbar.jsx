...
const Navbar = () => {
  ...
  return (
    <nav className="fixed w-full bg-gray-950/80 backdrop-blur-xl border-b border-gray-800 z-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          <div className="hidden md:flex items-center space-x-8">
            {navLinks.map((link) => (
              <Link
                key={link.name}
                to={link.path}
                className="text-gray-300 hover:text-teal-400 font-medium transition duration-200 relative group"
              >
                {link.name}
          ...
          <div className="hidden md:flex items-center space-x-4">
            ...
            <Link
              to="/register"
              className="px-5 py-2 bg-gradient-to-r from-teal-500 to-cyan-500 text-white rounded-lg hover:opacity-90 transition shadow-md hover:shadow-teal-500/20"
            >
              Sign Up
            </Link>
          ...
          <div className="px-4 py-4 space-y-3">
            ...
            {navLinks.map((link) => (
              <Link
                key={link.name}
                to={link.path}
                className="block px-4 py-2 text-gray-300 hover:bg-gray-800 hover:text-teal-400 rounded-lg transition"
                onClick={() => setIsMenuOpen(false)}
              >
                {link.name}
              </Link>
            ...
            <div className="pt-4 border-t border-gray-800 space-y-2">
              ...
              <Link
                to="/register"
                className="block px-4 py-2 bg-gradient-to-r from-teal-500 to-cyan-500 text-white text-center rounded-lg hover:opacity-90 transition"
                onClick={() => setIsMenuOpen(false)}
              >
                Sign Up
              </Link>
